...
const Register = () => {
  ...
  const verifyMobile = async () => {
    ...
    if (!mobile) {
      setMessage({ type: "error", text: "Enter a valid mobile number" });
      return;
    }
    try {
      ...
      if (!response.ok) {
        setMessage({
          type: "error",
          text: data.errors[0].msg || "Error sending verification SMS.",
        });
      ...
      setMessage({ type: "error", text: "Error sending verification SMS." });
      setMobileVerification(false);
      console.error("Error hitting End point:", err.message);
    }
  };
  ...
  const verifyMobileWithOtp = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage({
          type: "error",
          text: data.errors[0].msg || "Error verifying mobile number",
        });
        return;
      }
      setMessage({
        type: "success",
        text: data.msg || "Mobile number verified successfully",
      });
      setFormData((prev) => ({ ...prev, mobile_verified: true }));
    } catch (err) {
      setMessage({
        type: "error",
        text: err.message || "Error verifying mobile number",
      });
